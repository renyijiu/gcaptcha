...
static void line(unsigned char im[70*200], unsigned char swr[200], uint8_t s1) {
  ...
  for(x=0;x<199;x++) {
    ...
    int skew=sw[sk1]/11;
    sk1+=swr[x]&0x3+1;
    unsigned char *i= im+(200*(42+skew)+x);
...
void captcha(unsigned char im[70*200], unsigned char l[8], int length, int i_line, int i_filter) {
  ...
  read(f,l,5); read(f,swr,200); read(f,dr,sizeof(dr)); read(f,&s1,1); read(f,&s2,1); read(f,&s3,100); read(f,&s4,200);
  close(f);
...
void Init_gcaptcha() {
  GCaptcha = rb_define_module("GCaptcha");
  rb_define_singleton_method(GCaptcha, "create", create, 4);
}
